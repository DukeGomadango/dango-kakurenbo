...
export default function MatrixView({
  ...
  return (
    <>
      ...
      <div className={`${isBroadcastMode ? "block" : "hidden md:block"} h-full`}>
        <ResizableSplit
      ...
      <div className={`${isBroadcastMode ? "hidden" : "block md:hidden"} h-full relative`}>
        <div className={`glass-panel rounded-2xl p-4 flex flex-col overflow-hidden border border-white/10 select-none ${
          isBroadcastMode ? "h-[calc(100vh-120px)] min-h-[500px]" : "h-[calc(100vh-190px)] min-h-[510px]"
        }`}>
          {/* リストコントロールヘッダー */}
          <div className="space-y-3 pb-3 border-b border-white/5 shrink-0">
            <div className="flex justify-between items-center">
              ...
              <div className="flex flex-wrap items-center gap-2 p-2.5 rounded-xl bg-red-500/[0.04] border border-red-500/15 animate-fadeIn">
                ...
                <div className="flex items-center gap-1.5 ml-auto">
                  <button
                    ...
                    onClick={selectAllFilteredSuspects}
                    disabled={filteredSuspects.length === 0}
                    className="px-2 py-1 rounded-lg text-[10px] font-bold border border-white/10 text-[var(--text-secondary)] hover:text-[var(--foreground)] hover:bg-white/[0.05]"
                  >
                    全選択
                  </button>
                  <button
                    ...
                    onClick={handleBulkDeleteSuspects}
                    disabled={selectedSuspectIds.size === 0}
                    className="px-2.5 py-1 rounded-lg text-[10px] font-extrabold bg-red-500 hover:bg-red-600 text-white"
                  >
                    削除
                  </button>
                </div>
            ...
            {!isBroadcastMode && (
              <div className="relative animate-fadeIn">
                ...
                {suspectQuery && (
                  <button
                    onClick={() => setSuspectQuery("")}
                    className="absolute right-2 top-2 text-[var(--text-secondary)]"
                  >
                    <X size={12} />
                  </button>
                )}
                ...
                return (
                  ...
                  >
                    <div className="space-y-1.5 min-w-0 flex-1 pr-2">
                      <div className="flex items-start gap-2 min-w-0">
                        {suspectSelectMode && !isBroadcastMode && (
                          <button
                            ...
                            className="shrink-0 text-[var(--text-secondary)] hover:text-[var(--foreground)] mt-0.5"
                          >
                            {isBulkSelected ? <CheckSquare size={15} className="text-red-400" /> : <Square size={15} />}
                          </button>
                        )}
                        {suspect.isSolved ? (
                      ...
                      <div className="flex items-center gap-2 text-[9px] text-[var(--text-secondary)]">
                        <span className="shrink-0 font-medium flex items-center gap-0.5">
                          <MessageSquare size={9} />
                          {answeredQ}/{totalQ} 回答
                        </span>
                        {assignedRegulars.length > 0 && (
          ...
          <>
            ...
            <div
              className={`fixed inset-0 z-[60] bg-black/60 backdrop-blur-sm transition-opacity duration-300 ${
                isMobileDetailOpen ? "opacity-100 pointer-events-auto" : "opacity-0 pointer-events-none"
              }`}
            ...
            {/* ボトムシート本体 */}
            <div
              className={`fixed inset-x-0 bottom-0 z-[70] glass-panel rounded-t-3xl border-t border-white/12 max-h-[82vh] flex flex-col transition-transform duration-300 ease-out transform ${
                isMobileDetailOpen ? "translate-y-0" : "translate-y-full"
              }`}
              style={{
                boxShadow: `0 -10px 40px rgba(0,0,0,0.5), 0 0 0 1px rgba(var(--accent-r),var(--accent-g),var(--accent-b),0.15), var(--accent-glow)`,
            ...
            >
              ...
              {/* ヘッダーエリア */}
              <div className="px-5 pb-2.5 flex justify-between items-center border-b border-white/5 shrink-0">
                <span className="text-[10px] font-extrabold text-[var(--text-secondary)] uppercase tracking-wider">
                  参加者の詳細情報
                </span>
                <button
                  onClick={() => setIsMobileDetailOpen(false)}
                  className="p-1 rounded-full bg-white/5 border border-white/5 text-[var(--text-secondary)] hover:text-[var(--foreground)] cursor-pointer"
                >
                  <X size={15} />
                </button>
              </div>
              ...
              <div className="flex-1 overflow-y-auto min-h-0 p-5 space-y-4">
                ...
                <div className="space-y-3 pb-3 border-b border-white/5">
                  <div className="flex items-center gap-3">
                    <div className="w-8 h-8 rounded-lg flex items-center justify-center shrink-0 bg-[var(--accent-bg)] border border-[var(--accent-border)]">
                      <User size={18} style={{ color: "var(--accent-solid)" }} />
                    </div>
                    {isBroadcastMode ? (
                      <TruncatedText
                        text={selectedSuspect.fakeName}
                        multiline
                        className="broadcast-enlarge-text broadcast-suspect-name text-[var(--foreground)] font-bold select-text"
                      />
                    ) : (
                      <input
                        type="text"
                        className="bg-transparent border-none text-base font-extrabold text-[var(--foreground)] w-full outline-none focus:ring-0 p-0 border-b border-transparent focus:border-white/10"
                        value={selectedSuspect.fakeName}
                        onChange={(e) => handleUpdateSuspect(selectedSuspect.id, "fakeName", e.target.value)}
                        placeholder="偽名を入力..."
                      />
                  ...
                  <div className="flex items-center gap-2">
                    {isBroadcastMode ? (
                      selectedSuspect.isSolved && (
                        <span className="px-3.5 py-1.5 rounded-xl text-xs font-extrabold bg-emerald-500/10 border border-emerald-500/30 text-emerald-400 flex items-center gap-1.5 animate-pulse">
                          <CheckCircle2 size={13} />
                          本命当て完了
                        </span>
                      )
                      ...
                      <>
                        ...
                        <button
                          onClick={() => {
                            ...
                            handleDeleteSuspect(selectedSuspect.id);
                          }}
                          className="p-2 rounded-xl text-[var(--text-secondary)] hover:text-red-400 hover:bg-red-500/10 transition-colors border border-transparent hover:border-red-500/10 cursor-pointer"
                        >
                          <Trash2 size={15} />
                ...
                <div className="flex flex-col gap-2 text-xs pt-1">
                  <span className="text-[var(--text-secondary)] font-bold">本名予想:</span>
                  <div className="flex flex-wrap items-center gap-1.5 relative">
                    {(selectedSuspect.realNameGuesses || []).map(gName => {
                      ...
                      return (
                        <span
                          key={gName}
                          className="rounded-full font-bold border flex items-center shrink-0 px-2 py-0.5 text-[10px]"
                          style={{
                            backgroundColor: matchedReg ? `${matchedReg.color}15` : undefined,
                            borderColor: matchedReg ? `${matchedReg.color}35` : undefined,
                            color: matchedReg ? matchedReg.color : undefined,
                        ...
                        >
                          <span 
                            ...
                            style={{ backgroundColor: matchedReg?.color || "var(--accent-solid)" }}
                          />
                          <span className="ml-1">{gName}</span>
                          {!isBroadcastMode && (
                            <button
                              onClick={() => handleToggleGuess(selectedSuspect.id, gName)}
                              className="hover:bg-black/10 dark:hover:bg-white/10 rounded-full p-0.5 ml-1 transition-colors cursor-pointer"
                ...
                <div className="space-y-4 pt-3 border-t border-white/5">
                  <div className="text-[10px] font-bold text-[var(--text-secondary)] tracking-wider uppercase flex items-center gap-1.5 pb-1 border-b border-white/5 select-none">
                    <span>質問と回答</span>
                  </div>
                  ...
                  <div className="flex flex-col gap-3">
                    {questions.map((q, idx) => {
                      ...
                      return (
                        <div 
                          key={q.id}
                          className="glass-panel-light p-3.5 rounded-xl border border-white/5 flex flex-col gap-2 relative"
                        >
                          <div className="flex items-center justify-between gap-2 border-b border-white/5 pb-1.5">
                            <div className="flex items-center gap-1 flex-1">
                              <span className="font-bold text-[var(--text-secondary)] shrink-0 text-[10px]">
                                Q{idx + 1}:
                              </span>
                              {isBroadcastMode ? (
                                <TruncatedText
                            ...
                            {!isBroadcastMode && (
                              <button
                                onClick={() => handleRemoveQuestion(q.id)}
                                className="p-1 rounded text-red-400 hover:bg-red-500/10 transition-opacity cursor-pointer"
                                disabled={questions.length <= 1}
                              >
                                <X size={12} />
                              </button>
                          ...
                          {isBroadcastMode ? (
                            <div className="w-full bg-black/10 dark:bg-black/25 border border-white/5 rounded-lg px-3 py-2 text-[var(--foreground)] min-h-[50px] whitespace-pre-wrap select-text shadow-sm border-violet-500/10">
                              {value || "（未回答）"}
                            </div>
                          ) : (
                            <textarea
                  ...
                  {!isBroadcastMode && (
                    <button
                      onClick={handleAddQuestion}
                      className="w-full py-3 rounded-xl border border-dashed border-white/10 text-xs font-bold text-[var(--text-secondary)] hover:text-[var(--foreground)] flex items-center justify-center gap-2 hover:bg-white/[0.01] transition-all cursor-pointer"
                    >
                      <Plus size={14} style={{ color: "var(--accent-solid)" }} />
                      質問を追加する
                    </button>
                  )}
                </div>
